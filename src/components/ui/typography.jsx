...
export function Typography({ children, variant = "p", className = "" }) {
  switch (variant) {
    ...
    case "h2":
      return (
        <h2 className={`scroll-m-20 pb-2 text-3xl font-semibold tracking-tight ${className}`}>
          {children}
        </h2>
      );
    case "h3":
    ...
    case "blockquote":
      return (
        <blockquote className={`mt-2 border-l-2 pl-2 italic ${className}`}>{children}</blockquote>
      );
    case "lead":
    ...
    case "small":
      return <small className={`text-sm leading-none ${className}`}>{children}</small>;
    case "muted":
