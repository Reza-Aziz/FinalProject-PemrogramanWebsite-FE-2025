...
import { Play, Gamepad2, Image as ImageIcon } from "lucide-react";
...
const MainMenu = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative">
      {/* Fixed Background with Gradient and Blobs */}
      ...
      <div className="z-10 w-full max-w-4xl flex flex-col gap-8 animate-in fade-in zoom-in duration-700 py-10">
        ...
        {/* Create Game Section (Collapsible) */}
        ...
        {/* Game List (Only visible if NOT in create mode) */}
        {!isCreateMode && (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {games.map(game => (
                                    <Link key={game.id} to={`/play/${game.id}`}>
                                        <div className="bg-white rounded-xl p-4 flex gap-4 items-center shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all cursor-pointer border-2 border-transparent hover:border-yellow-400 group">
